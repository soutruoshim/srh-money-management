...
class Dashboard extends StatelessWidget {
  ...
  void removeWallet(BuildContext context, WalletModel walletModel) {
    context.read<WalletsBloc>().add(RemoveWallet(walletModel));
                    ...
                    child: Column(
                      children: [
                        ...
                        Padding(
                          padding: const EdgeInsets.only(bottom: 16),
                          child: Text(
                            state.totalBalanceAllWallets == 0
                                ? '${BlocProvider.of<UserBloc>(context).userModel?.currencySymbol ?? '\$'}0.00'
                                : '${state.totalBalanceAllWallets > 0 ? "" : "-"}${BlocProvider.of<UserBloc>(context).userModel?.currencySymbol ?? '\$'}${formatMoney.format(state.totalBalanceAllWallets.abs())}',
